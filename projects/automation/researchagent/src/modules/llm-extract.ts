...
import { SearchConfig, ExtractionConfig } from '../core/schema-loader.js';
import {
...
export async function extractWithLLM(
  ...
  try {
    console.log(`[LLM] Extracting from: ${data.url} (model: ${model})`);

    // 初期情報種別の判定（スキーマから primary_info_domains を取得）
    ...
    for (let attempt = 0; attempt < retryAttempts; attempt++) {
      try {
        ...
        // JSONパース
        const parsed = JSON.parse(content) as CaseStudyRecord;
        ...
        lastError = error;
        console.warn(`[LLM] Attempt ${attempt + 1}/${retryAttempts} failed:`, error.message);
    ...
    console.error(`[LLM] Failed to extract after ${retryAttempts} attempts:`, lastError.message);
    return null;
  } catch (error: any) {

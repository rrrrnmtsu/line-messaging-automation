...
import { fetchAndExtractHybrid } from './modules/fetch-hybrid.js';
...
program
  ...
  .version('1.0.0')
  .option('--schema <name>', 'リサーチスキーマ名 (例: n8n-case-study, saas-comparison)', 'n8n-case-study')
  ...
  .option('--export-notion', 'Notion にエクスポート')
  .option('--notion-database-id <string>', 'Notion データベース ID')
  .option('--notion-parent-page-id <string>', 'Notion 親ページ ID（新規DB作成用）')
  .parse(process.argv);
...
const schema = schemaLoader.loadSchema(schemaName);
...
// スキーマから検索設定を取得（CLIオプションで上書き可能）
const concurrency = options.concurrency ? parseInt(options.concurrency, 10) : (searchConfig.concurrency || 6);
const perQuery = options.perQuery ? parseInt(options.perQuery, 10) : (searchConfig.per_query || 20);

const exportNotion = options.exportNotion || false;
const notionDatabaseId = options.notionDatabaseId || process.env.NOTION_DATABASE_ID || '';
const notionParentPageId = options.notionParentPageId || process.env.NOTION_PARENT_PAGE_ID || '';

console.log(`
==============================================
n8n Research Agent (Schema-Driven)
==============================================
Schema: ${schemaName}
Phase: ${phase}
...
Concurrency: ${concurrency} (from schema)
Per Query: ${perQuery} (from schema)
Keywords: ${searchConfig.base_keywords.length} keywords
==============================================
`);
...
async function main() {
  ...
  // スキーマからキーワードを取得
  const allQueries = searchConfig.base_keywords;

  // ハイブリッド検索: WebSearch MCP + DuckDuckGo（スキーマ設定を渡す）
  ...
  console.log(`[Step 1] ハイブリッド検索完了: ${searchResults.length}件のURL`);

  // ========== 2. 取得・抽出・LLM処理（ハイブリッド取得） ==========
  console.log('\n[Step 2] ハイブリッド取得・LLM抽出開始...');
  ...
  await Promise.all(
    searchResults.map((result) =>
      fetchLimit(async () => {
        ...
        try {
          // ハイブリッド取得: axios → Puppeteerフォールバック
          const extracted = await fetchAndExtractHybrid(result.url);

          if (!extracted) {
            logEntry.status = 'skipped';
            logEntry.reason = 'Content too short or fetch failed (both axios and Puppeteer)';
  ...
  // Notion エクスポート
  if (exportNotion) {
    console.log('\n[Step 5] Notion エクスポート開始...');

    if (!notionDatabaseId && !notionParentPageId) {
      console.error('エラー: NOTION_DATABASE_ID または NOTION_PARENT_PAGE_ID が必要です');
      console.error('オプション指定: --notion-database-id <ID> または --notion-parent-page-id <ID>');
      console.error('環境変数指定: NOTION_DATABASE_ID または NOTION_PARENT_PAGE_ID');
    } else {
      const { exportToNotion, createNotionDatabase, validateNotionDatabase } = await import('./modules/notion-export.js');

      let targetDatabaseId = notionDatabaseId;

      // データベース ID がない場合は新規作成
      if (!targetDatabaseId && notionParentPageId) {
        console.log('[Notion Export] 新規データベースを作成します...');
        const newDbId = await createNotionDatabase(schemaName, notionParentPageId, `${schemaName}_${outPrefix}`);
        if (newDbId) {
          targetDatabaseId = newDbId;
          console.log(`[Notion Export] データベース作成成功: ${newDbId}`);
        } else {
          console.error('[Notion Export] データベース作成失敗');
        }
      }
      ...
      if (targetDatabaseId) {
        // スキーマ検証
        const isValid = await validateNotionDatabase(schemaName, targetDatabaseId);
        if (!isValid) {
          console.warn('[Notion Export] 警告: データベーススキーマが不完全な可能性があります');
        }

        // エクスポート
        const exportedCount = await exportToNotion(schemaName, finalRecords, targetDatabaseId);
        console.log(`[Notion Export] ${exportedCount}件のレコードをエクスポートしました`);
      }
    }
  }
